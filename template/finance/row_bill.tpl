...
<td>
    <div class="order-num">{$oLanguage->GetMessage('cartpackage #')}</div>
    <a href="/?action=cart_package_list?search[id]={$aRow.id_cart_package}">{$aRow.id_cart_package}</a>
</td>
{/if}
{else}
<td></td>
...
<td nowrap>
{if $smarty.request.action!='finance_reestr_bv'}
...
{/if}
{if $aRow.id_bill} 
	<a href="/?action=finance_bill_edit&id={$aRow.id}&return_action={$smarty.request.action}"
	><img src="/image/edit.png" border=0 width=16 align=absmiddle hspace=1/>{$oLanguage->getMessage("Account Edit")}</a>
	<br>
	<a href="/?action=finance_bill_delete&id={$aRow.id}&return_action={$smarty.request.action}"
