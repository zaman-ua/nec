...
var obj =$(this);
if(obj.prop("checked")){$('input[name^=row_check]').prop('checked',true)}
else{$('input[name^=row_check]').prop('checked',false);}
});


$( 'body' ).on( "keydown", "#filter_input_id", function(event) {
	  if (event.keyCode == 13) { //если нажали Enter, то true
$('#find_by_filter').click();
		  }
});
